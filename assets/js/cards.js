...
function createCard(data) {
  ...
  card.innerHTML = `
    ...
    <div class="detail-grid">
      ...
      <p class="territory">
        Territory: ${formatTerritory(data.territory)}
      </p>
      ...
      <p class="local-stock">
        ${
          data.localStock
            ? '<img src="assets/images/manufacturers/ad-small.png" alt="AD logo">'
            : ''
        }
      </p>
    </div>
    ...
    <div class="button-row">
      <button class="website-btn" type="button">
        Website
      </button>
...
function renderManufacturers(dataArray, stateFilter = 'all') {
  ...
  if (resultsCount) {
    resultsCount.textContent =
      `Showing ${filteredData.length} manufacturer${filteredData.length === 1 ? '' : 's'}`;
  }
